...
function EyeIcon({
  size = DEFAULT_PASSWORD_ICONS_SIZE,
  color = DEFAULT_PASSWORD_ICONS_COLOR,
}: IIconProps) {
  return (
    ...
    <View style={[styles.container, containerStyle]}>
      ...
      <View
        ...
        <TextInput
          ...
          {...rest}
        />
        {endIcon && <View style={[styles.startIconContainer]}>{endIcon}</View>}
        {isPasswordField && (
          <Pressable
            hitSlop={{ bottom: 50, left: 50, top: 50, right: 50 }}
            onPress={() => setIsPasswordHidden(!isPasswordHidden)}>
            {isPasswordHidden ? (
              <>{showPasswordIcon ?? <EyeIcon color="rgba(0,0,0,.25)" />}</>
            ) : (
              <>
                {hidePasswordIcon ?? <EyeSlashIcon color="rgba(0,0,0,.25)" />}
              </>
            )}
          </Pressable>
        )}
